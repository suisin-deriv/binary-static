...
const ConvertToBase64 =
  require("../../../../_common/image_utility").convertToBase64;
...
const showLoadingImage =
  require("../../../../_common/utility").showLoadingImage;
...
const Authenticate = (() => {
  ...
  const init = () => {
    ...
    const language_based_link = ["ID", "RU", "PT"].includes(language)
      ? `_${language}`
      : "";
    const $not_authenticated = $("#not_authenticated");
    const link = Url.urlForCurrentDomain(
      `https://marketing.binary.com/authentication/Authentication_Process${language_based_link}.pdf`
    );
   ...
   */
  const isIdentificationNoExpiry = (residence) =>
    /(ng|za|lk|in|sg|id|mm|vn|br|mx|co)/.test(residence);
  ...
  const onFileSelected = (event) => {
    ...
    const display_name =
      file_name.length > 20
        ? `${file_name.slice(0, 10)}..${file_name.slice(-8)}`
        : file_name;
  ...
  const enableDisableSubmit = () => {
    const $not_authenticated = $(
      "#authentication-message > div#not_authenticated"
    );
  ...
  const showButtonLoadingUns = () => {
    if ($button_uns.length && !$button_uns.find(".barspinner").length) {
      const $btn_text = $("<span/>", {
        text: $button_uns.find("span").text(),
        class: "invisible",
      });
    ...
    $files.each((i, e) => {
      if (e.files && e.files.length) {
        ...
        if (/front|back/.test(id)) {
          display_name += ` - ${
            /front/.test(id) ? localize("Front Side") : localize("Reverse Side")
          }`;
        }

        $submit_table.append(
          $("<tr/>", { id: file_obj.type, class: id })
  ...
  const processFiles = (files) => {
    ...
    readFiles(files).then((files_to_process) => {
      ...
      compressImageFiles(files_to_process).then((processed_files) => {
        ...
        const uploadFile = () => {
          const $status = $submit_table.find(
            `.${processed_files[idx_to_upload].passthrough.class} .status`
          );
          $status.text(`${localize("Submitting")}...`);
          uploader
            .upload(processed_files[idx_to_upload])
            .then((api_response) => {
              onResponse(api_response, isLastUpload());
              if (!api_response.error && !api_response.warning) {
                $status
                  .text(localize("Submitted"))
                  .append($("<span/>", { class: "checked" }));
                $(`#${api_response.passthrough.class}`).attr("type", "hidden"); // don't allow users to change submitted files
                $(`label[for=${api_response.passthrough.class}]`)
                  .removeClass("selected error")
                  .find("span")
                  .attr("class", "checked");
              }
  ...
  const processFilesUns = (files) => {
    ...
    compressImageFilesUns(files).then((files_to_process) => {
      readFilesUns(files_to_process).then((processed_files) => {
        ...
        // sequentially send files
        const uploadFile = () => {
          const $status = $submit_table_uns.find(
            `.${processed_files[idx_to_upload].passthrough.class} .status`
          );
          $status.text(`${localize("Submitting")}...`);
          uploader
    ...
    files.forEach((f) => {
      const promise = new Promise((resolve) => {
        if (isImageType(f.filename)) {
          ...
          const $filename = $submit_table.find(
            `.${f.passthrough.class} .filename`
          );
  ...
  const fileTracker = ($e, selected) => {
    const doc_type = ($e.attr("data-type") || "")
      .replace(/\s/g, "_")
      .toLowerCase();
  ...
  const onErrorResolved = (error_field, class_name, reverse_class_name) => {
    const id = error_field
      ? `${error_field}_${class_name.match(/\d+/)[0]}`
      : reverse_class_name;
    $(`#${id}`).one("input change", () => {
  ...
  const validate = (file) => {
    ...
    if (
      ...
      !(
        ...
        file.documentType === "national_identity_card"
      )
    ) {
  ...
  const initOnfidoVerification = async () => {
    ...
    if (
      ...
      const missing_personal_fields = Object.keys(
        service_token_response.error.details
      ).map((field) => personal_fields_errors[field].toLowerCase() || field);

      const error_msgs = missing_personal_fields
        ? missing_personal_fields.join(", ")
        : "";
    ...
    if (has_personal_details_error) {
      ...
      const onfido_documents = Array.isArray(documents_supported)
        ? documents_supported
        : Object.keys(documents_supported).map(
            (d) => documents_supported[d].display_name
          );
          ...
          steps: [
            {
              ...
              options: {
                documentTypes: {
                  passport: onfido_documents.some((doc) =>
                    /Passport/g.test(doc)
                  ),
                  driving_licence: onfido_documents.some((doc) =>
                    /Driving Licence/g.test(doc)
                  )
                    ? {
  ...
  const showCTAButton = async (type, status) => {
    BinarySocket.send({ get_account_status: 1 }, { forced: true }).then(
      (response) => {
        ...
        $(
          `#text_verified_${type_pending}_required, #text_pending_${type_pending}_required`
        ).setVisibility(0);
        $(
          `#button_verified_${type_pending}_required, #button_pending_${type_pending}_required`
        ).setVisibility(0);
  ...
  const getOnfidoServiceToken = (country_code) =>
    new Promise((resolve) => {
  ...
  const handleCountrySelector = async () => {
    ...
    const is_idv_disallowed = account_status.status.some(
      (ac) => ac === "idv_disallowed"
    );

    selected_country = null;
    await BinarySocket.send({ residence_list: 1 }).then(
      (response) => (residence_list = response.residence_list)
    );
    ...
    if (residence_list.length > 0) {
      ...
      $residence_dropdown.on("change", (e) => {
        const dropdown_country = residence_list.find(
      ...
      next_button.addEventListener("click", () => {
        if (selected_country) {
          ...
          if (
            ...
            is_onfido_supported &&
            Number(onfido_submissions_left) > 0
          ) {
  ...
  const handleIdvDocumentSubmit = async () => {
    ...
    const example_format_text = document.getElementById(
      "document_example_format"
    );
    const document_input = document.getElementById("document_number");
    const document_sample_img = document.getElementById(
      "document_sample_image"
    );
    const back_button = document.getElementById("idv_document_submit_back_btn");
    const verify_button = document.getElementById(
      "idv_document_submit_verify_btn"
    );
    ...
    const country_code = selected_country.value;
    const { has_visual_sample, documents_supported } =
    ...
    available_document_list = filtered_documents.map((d) => {
      const { new_display_name, example_format, sample_image } =
        getDocumentData(country_code, d) || {};
    ...
    if (selected_country) {
      ...
      $documents.on("change", (e) => {
        document_type = available_document_list.find(
  ...
  const handleIdv = async () => {
    ...
    const { submissions_left: onfido_submissions_left } = onfido;
    const needs_poa =
      account_status.authentication.needs_verification.length &&
      account_status.authentication.needs_verification.includes("document");
    $("#idv-container").setVisibility(1);
  ...
  const initAuthentication = async () => {
    ...
    const is_fully_authenticated =
      identity.status === "verified" && document.status === "verified";
